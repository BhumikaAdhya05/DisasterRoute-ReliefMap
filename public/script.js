...
let routeLayer, blockedLayer, originalRouteLayer;
let simulationInterval, simulationIndex = 0;
let isRerouted = false, reroutePath = [], movingMarker;
let blockedPolygons = [], reroutedRouteCoords = [];
...
async function getRoute() {
  ...
  let blocked = [];
  try {
    if (blockedRaw.trim()) blocked = JSON.parse(blockedRaw);
  } catch {
    alert("Invalid blocked roads JSON format.");
    return;
  }
  ...
  try {
    const [originalRes, reroutedRes] = await Promise.all([
      fetch('/api/route', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ start, end })
    ...
    [routeLayer, blockedLayer, originalRouteLayer, movingMarker].forEach(layer => {
  ...
  simulationInterval = setInterval(() => {
    ...
    const isBlocked = blockedZones.some(poly => {
      try {
        ...
        console.warn("Polygon skipped due to error:", err);
        return false;
      }
    });
...
async function fetchReroute(currentCoord, endCoord) {
  try {
    ...
    if (!data.features || !data.features[0]) {
      ...
      const fallbackCoords = allLatLng.slice(startIdx);
      if (fallbackCoords.length > 1) {
        L.polyline(fallbackCoords, { color: 'green', weight: 4 }).addTo(map);
        simulationIndex = 0;
        simulateMovement(fallbackCoords, blockedPolygons, endCoord);
